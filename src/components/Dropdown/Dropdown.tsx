...
const Container = styled.div<{maxWidth?: string, minWidth?: string}>`
  ...
  ${space}
`;
const DropdownTop = styled.div<{ disabled?: boolean, scale?: string, variant?: string }>`
  display: flex;
  align-items: center;
  width: 100%;
  border: 1px solid;
...
const Dropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <Container maxWidth={maxWidth} minWidth={minWidth} ref={wrapperRef} {...props}>
      <DropdownTop scale={scale} variant={variant} onClick={toggling} disabled={disabled} className={isOpen ? 'open' : disabled ? 'disabled' : ''}>
        {selectedOption.icon &&
      ...
      {isOpen &&
        <DropdownContent position={position} scale={scale}>
          {options.map((option) => (
            <DropdownItem scale={scale} selected={option.label === selectedOption.label} onClick={onOptionClicked(option)} key={option.label}>
              {option.icon &&
