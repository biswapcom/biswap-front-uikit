...
const Container$3 = ({ children, ...props }) => (React__default["default"].createElement(Box, { width: "100%", px: getResponsiveAttrs({ xs: "16px", lg: "24px", xxl: 0 }), mx: "auto", maxWidth: "1120px", ...props }, children));
...
const getBottom$1 = ({ position }) => {
    if (position === "top") {
        return "calc(100% + 8px)";
    }
    return "-8px";
};
const Container$1 = styled__default["default"].div.attrs({ position: "bottom" }) `
  position: relative;
  width: 100%;
  ...
  ${styledSystem.space}
`;
const DropdownTop$1 = styled__default["default"].div `
  display: flex;
  align-items: center;
  width: 100%;
  height: 100%;
...
`;
const DropdownItem$1 = styled__default["default"].div `
  display: flex;
  align-items: center;
...
`;
const Dropdown = ({ position = "bottom", children, maxWidth, minWidth, scale, variant, disabled, options, onChange, selectedItem, ...props }) => {
    ...
    const scaleVariantsImage = (scale) => {
        switch (scale) {
            case scales$5.LG:
                return 24;
            case scales$5.MD:
...
`;
const DropdownButton = ({ position = "bottom", children, maxWidth, minWidth, scale = SCALES.MD, variant = VARIANTS.PRIMARY, disabled = false, options, onChange, hideLabel, dropDownWidth, selectedItem, ...props }) => {
    const [isOpen, setIsOpen] = React.useState(false);
    const [selectedOption, setSelectedOption] = React.useState(selectedItem || options[0]);
    const wrapperRef = React.useRef(null);
...
const MenuItems = ({ items = [], activeItem, activeSubItem, isMobileMenuOpened = false, mobileMenuCallback, baseAwsUrl, children, mobileLangSelector, ...props }) => {
...
//eventButtonLogo,
customLogoSubtitle, marketplaceLink, baseAwsUrl = "https://static.biswap.org/bs", buyBswLabel = "Buy BSW", mobileLangSelector, }) => {
    const { isMobile } = useMatchBreakpoints();
